...
text_schools <- function(analysis_results) {
  ...
  rate_range_fmt         <- paste0(round(rate_range * 100, 1), " percentage points")
  ...
  if (top_20_count == 1) {
    if (total_schools == 1) {
      ...
      concentration_text <- paste0(
        "The highest-impact school accounts for ",
    ...
    concentration_text <- paste0(
      ...
      top_20_pct_share_fmt,
      " of the district's chronic absence."
    )
  }
  ...
  if (total_schools == 1) {
    ...
    if (schools_for_30 == 1) {
      impact_text <- paste0(
        "The single highest-impact school accounts for 30% of district chronic absence, while the top ",
        schools_for_50,
  ...
  if (nchar(distribution_text) > 0) {
    schools_text <- paste0(schools_text, " ", distribution_text)
  }
  
  if (nchar(impact_text) > 0) {
    schools_text <- paste0(schools_text, " ", impact_text)
  }
