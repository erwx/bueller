...
render_district <- function(data_file, district_name) {
  ...
  qmd_content <- c(
    ...
    "  trends_overall$YOY_RATE_CHANGE <- ifelse(is.na(trends_overall$YOY_RATE_CHANGE), '-',",
    "    paste0(ifelse(trends_overall$YOY_RATE_CHANGE >= 0, '+', ''), round(trends_overall$YOY_RATE_CHANGE * 100, 1), 'pp'))",
    "  ",
    "  knitr::kable(trends_overall[, c('YEAR', 'TOTAL_STUDENTS', 'CHRONIC_ABSENT', 'CHRONIC_RATE', 'YOY_RATE_CHANGE')],",
