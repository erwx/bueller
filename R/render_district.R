...
render_district <- function(data_file, district_name) {
  ...
  qmd_content <- c(
    ...
    "```",
    "",
    "## District Trends by Year",
    "",
    "```{r district-trends-table}",
    ...
    "  trends_overall$TOTAL_STUDENTS <- format(trends_overall$TOTAL_STUDENTS, big.mark = ',')",
    "  trends_overall$CHRONIC_ABSENT <- format(trends_overall$CHRONIC_ABSENT, big.mark = ',')",
    "  ",
    ...
    "               caption = 'District Chronic Absence by Year')",
    "} else {",
    "  cat('Multi-year trend data not available.')",
    "}",
    "```",
    "",
    "## Student Subgroup Trends by Year",
    "",
    "```{r subgroup-trends-table}",
    "if (analysis_results$trends$has_trends && length(analysis_results$trends$demographic_trends) > 0) {",
    "  # Create comprehensive subgroup trends table",
    "  all_subgroup_data <- data.frame()",
    "  ",
    "  for (group_name in names(analysis_results$trends$demographic_trends)) {",
    "    group_data <- analysis_results$trends$demographic_trends[[group_name]]",
    "    group_data$GROUP <- group_name",
    "    all_subgroup_data <- rbind(all_subgroup_data, group_data)",
    "  }",
    "  ",
    "  if (nrow(all_subgroup_data) > 0) {",
    "    # Format the data",
    "    all_subgroup_data$CHRONIC_RATE <- paste0(round(all_subgroup_data$CHRONIC_RATE * 100, 1), '%')",
    "    all_subgroup_data$RATE_CHANGE <- ifelse(is.na(all_subgroup_data$RATE_CHANGE), '-',",
    "      paste0(ifelse(all_subgroup_data$RATE_CHANGE >= 0, '+', ''), round(all_subgroup_data$RATE_CHANGE * 100, 1), 'pp'))",
    "    all_subgroup_data$TOTAL_STUDENTS <- format(all_subgroup_data$TOTAL_STUDENTS, big.mark = ',')",
    "    all_subgroup_data$CHRONIC_ABSENT <- format(all_subgroup_data$CHRONIC_ABSENT, big.mark = ',')",
    "    ",
    "    # Reorder columns",
    "    subgroup_table <- all_subgroup_data[, c('GROUP', 'YEAR', 'TOTAL_STUDENTS', 'CHRONIC_ABSENT', 'CHRONIC_RATE', 'RATE_CHANGE')]",
    "    ",
    "    knitr::kable(subgroup_table,",
    "                 col.names = c('Student Group', 'Year', 'Total Students', 'Chronic Absent', 'Rate', 'Year-over-Year Change'),",
    "                 caption = 'Student Subgroup Chronic Absence by Year')",
    "  } else {",
    "    cat('Subgroup trend data not available.')",
    "  }",
    "} else {",
    "  cat('Multi-year subgroup trend data not available.')",
    "}",
    "```"
  )
